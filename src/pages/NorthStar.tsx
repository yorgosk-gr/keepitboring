...
import { Compass, Plus, Trash2, Pencil, Check, X, ArrowRight, Loader2, Import } from "lucide-react";
...
export default function NorthStar() {
  ...
  const [newPos, setNewPos] = useState({
    ...
    status: "build" as const, priority: 2, rationale: "",
  });

  // Alignment calculations
  const alignmentData = useMemo(() => {
    if (nsPositions.length === 0) return { score: 0, gaps: [], cashNeeded: 0 };

    const currentMap: Record<string, number> = {};
    for (const p of currentPositions) {
      currentMap[p.ticker] = p.weight_percent ?? 0;
    }

    let totalGap = 0;
    let totalWeight = 0;
    const gaps = nsPositions.map((ns) => {
      const current = currentMap[ns.ticker] ?? 0;
      const ideal = ns.target_weight_ideal ?? 0;
      const gap = Math.abs(current - ideal);
      totalGap += gap;
      totalWeight += ideal;
      return { ticker: ns.ticker, current, ideal, gap, status: ns.status };
    }).sort((a, b) => b.gap - a.gap);

    const score = totalWeight > 0 ? Math.max(0, Math.round(100 - (totalGap / totalWeight) * 50)) : 0;

    // Cash needed: sum of (ideal - current) for build positions * total portfolio value
    const totalValue = currentPositions.reduce((s, p) => s + (p.market_value ?? 0), 0);
    const cashNeeded = gaps
      .filter((g) => g.status === "build" && g.current < g.ideal)
      .reduce((s, g) => s + ((g.ideal - g.current) / 100) * totalValue, 0);
  ...
  const handleAddPosition = async () => {
    ...
    setNewPos({ ticker: "", name: "", target_weight_ideal: "", target_weight_min: "", target_weight_max: "", status: "build", priority: 2, rationale: "" });
    setShowAdd(false);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <div className="flex gap-3">
          <Button onClick={handleImport} disabled={isImporting || currentPositions.length === 0} className="gap-2">
          ...
          {isLoadingPositions ? (
            ...
            <div className="rounded-lg border border-border overflow-hidden">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {nsPositions.map((pos) => {
                    const currentWeight = currentPositions.find(p => p.ticker === pos.ticker)?.weight_percent ?? 0;
                    const isEditing = editingId === pos.id;
                    const sc = statusConfig[pos.status];
                    return (
                      <tr key={pos.id} className="border-t border-border hover:bg-secondary/20 transition-colors">
                        ...
                        <td className="px-3 py-2 text-right text-muted-foreground">{currentWeight.toFixed(1)}%</td>
